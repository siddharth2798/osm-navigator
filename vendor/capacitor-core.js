// Vendored from the `@capacitor/core` npm package (v8.5.0), dist/index.js,
// unmodified apart from this header and dropping the trailing sourcemap
// comment (the .map file itself isn't vendored). MIT licensed (Ionic).
//
// This app has no build step, so nothing ever actually imports
// @capacitor/core's real JS runtime — the Android native side only injects
// a minimal window.Capacitor stub (isNativePlatform/getPlatform), which
// does NOT include registerPlugin(). native-location.js's use of
// window.Capacitor.registerPlugin() throws ReferenceError-adjacent
// ("registerPlugin is not a function") without this: importing this file
// at all re-creates window.Capacitor as the fully-capable object (see
// initCapacitorGlobal below, which runs as a module-load side effect and
// merges in whatever the native bridge already provided) and exports
// registerPlugin directly for native-location.js to call instead of going
// through the global.
//
// Confirmed live on a real Android build (see task history): before this
// fix, startLocationWatch() failed with exactly
