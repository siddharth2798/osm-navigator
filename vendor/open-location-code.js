...
//
// Vendored from the `open-location-code` npm package (v1.0.3), unmodified
// apart from this notice and the final export statement — the upstream file
// only exposes a CommonJS `exports.OpenLocationCode`, which doesn't run as a
// browser <script> or an ES module, so the last line is rewritten as a
// standard `export` instead. Used by resolveGoogleMapsLink in app.js to
// decode the Plus Code Google embeds in a place's name (e.g. "R72F+2J
// Chellanam Sea Wall") when the place has no street address and Google's
// resolved URL carries no other coordinates.
