...
- **`VALHALLA_URL`** — the public default is a [shared demo server](https://valhalla.github.io/demos/routing/), not an SLA-backed API. Self-hosting means building a routing graph from an OSM extract. Worth knowing if you're on the public default: a single Kochi transit plan now searches multiple itinerary candidates (see "Multiple itinerary alternatives" in `docs/KOCHI_TRANSIT.md`) and can issue roughly 6-9 real Valhalla requests, not just 1-2 — deduped from a naive ~12 thanks to `cachedDriveOrWalkLeg`, but still noticeably more per plan than drive/walk mode.
- **`USE_SELF_HOSTED_VALHALLA`** — true by default but harmless on its own: the real gate is `SELF_HOSTED_VALHALLA_URL`, which lives **only** as a Cloudflare secret/variable, never in `config.js` (a personal server's address doesn't belong in a file shipped to every visitor). Set it via `wrangler secret put SELF_HOSTED_VALHALLA_URL` (plain Worker) or your Pages project's Settings → Environment variables (as a secret). The app calls this deployment's own `/api/valhalla-route`/`/api/valhalla-height`/`/api/valhalla-trace_attributes` (see `lib/valhalla-proxy.js`), which attach the real address server-side; if the secret isn't set, those return `501` and the app falls back to `VALHALLA_URL`. The in-app **Settings → Self-hosted Valhalla** toggle overrides this per device. `SELF_HOSTED_VALHALLA_COVERAGE_BBOX` isn't sensitive and stays in `config.js` — but it defaults to this project's maintainer's own extract coverage (greater Kochi), not a generic default. Change it (or set it `null`) before relying on this elsewhere, or your own self-hosted instance will just never get tried.
...
- **`TOMTOM_FEATURES_ENABLED`** — optional, same pattern as Valhalla/Open Charge Map above: gates two independent features — live traffic (route-option comparison at planning time plus in-drive check-ins, TomTom Flow Segment Data, 20K free requests/month — see the `TRAFFIC_*` tunables in `config.js`) and a Places Search fallback for category searches that come back empty from Nominatim (5K–10K free requests/month depending on endpoint). Planning-time comparison samples every alternate shown, so a trip with 2–3 meaningfully different options can use a noticeable slice of that free tier faster than the in-drive check-ins alone — though a short-lived cache (`TRAFFIC_CACHE_TTL_MS`/`TRAFFIC_CACHE_GRID_DECIMALS`) collapses repeat queries to virtually the same spot (options sharing a stretch near a common start/end point, a slow-moving check-in re-polling the same jam) into one real call, so this costs less than the raw sample counts alone would suggest. If the fastest option is congested somewhere Valhalla's own alternates don't route around, planning-time comparison also tries forcing Valhalla past that stretch via `exclude_polygons` (see `TRAFFIC_DETOUR_BUFFER_M`) and — only if the result is genuinely faster — adds it as its own "Avoids traffic" card; this is a whole extra Valhalla + TomTom round trip, so it only fires when there's real congestion to route around, not on every plan. Doesn't hold an API key itself — the real key lives only as a Cloudflare secret (`TOMTOM_API_KEY`, read server-side by `lib/tomtom-traffic-proxy.js`/`lib/tomtom-places-proxy.js`). Get a free key at [developer.tomtom.com](https://developer.tomtom.com), set it as that secret, deploy, then flip this flag to `true`. Leave `false` to disable both features entirely — no calls to `/api/traffic` or `/api/places` at all. The in-app **Settings → TomTom live traffic** toggle overrides this per device, same pattern as the Self-hosted Valhalla toggle above — though turning it on there does nothing if this deployment never configured the secret. Works on both deployment paths — `lib/tomtom-traffic-proxy.js`/`lib/tomtom-places-proxy.js` are shared between `worker.js` (the plain Cloudflare Worker path) and `functions/api/traffic.js`/`functions/api/places.js` (Cloudflare Pages).
