...
**CI-built APK** (`.github/workflows/daily-release.yml`): builds and attaches `osm-navigator.apk` to each day's release automatically — this is what the download link above always points at. Signs with the same keystore you'd use locally, read from four repo secrets so the keystore itself is never committed:
