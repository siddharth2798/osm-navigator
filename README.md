...
- **EV charging station details**, when `OPENCHARGEMAP_API_KEY` is configured — connector type, power, operator, cost, and an honestly-labeled operational status ("Reported operational · checked 4 months ago", not a live "in use" signal — see the config comment for why). Falls back to a plain OSM pin with no detail otherwise.
...
- `USE_SELF_HOSTED_VALHALLA` — set `true` to route through `SELF_HOSTED_VALHALLA_URL` instead of (or, with `SELF_HOSTED_VALHALLA_COVERAGE_BBOX` set, alongside) `VALHALLA_URL` above. Off by default, so it's a quick flip for testing a self-hosted instance without permanently switching the app over to it — see the comment block above it in `config.js` for the coverage-bbox fallback behavior.
- `MAPILLARY_ACCESS_TOKEN`, `OTP2_URL` — both optional; leave empty to disable street-level imagery / transit mode respectively.
- `OPENCHARGEMAP_API_KEY` — optional, free at [openchargemap.org](https://openchargemap.org/site/develop/api); leave empty and EV charging search stays a plain OSM pin with no connector/power/cost/status detail. Not a live "is it free right now" feature — see the config comment for why (short version: that data sits behind OCPI, which requires a registered business relationship with each charging network, not something a personal project can get).
...
- **EV charging search shows an error, or silently reverts to plain OSM pins**: an empty/invalid/expired `OPENCHARGEMAP_API_KEY` makes Open Charge Map's API reject the request outright (it has no anonymous/demo access at all, confirmed live) — check the key at [openchargemap.org](https://openchargemap.org/site/develop/api). With the key left empty, this is expected, silent, and not a bug: the app falls back to the same OSM-based search it always used before this feature existed.
...
- **EV charging search is genuinely sparse** in India's current OSM coverage — set `OPENCHARGEMAP_API_KEY` for real connector/power/operator/cost/status detail from a dedicated EV charging database instead. Even then, "status" is Open Charge Map's own community-maintained operational flag (working/broken/planned), not live occupancy — no data source accessible to a personal project actually has that; see the config comment for why OCPI/OCPP don't help.
