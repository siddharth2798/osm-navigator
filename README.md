...
- **Directions** — multi-stop routing (up to 8, drag to reorder, including dragging a stop above the start or below the destination to promote it), a plain-text "X to Y" shortcut, Drive/Walk/Transit modes, avoid-tolls/avoid-highways. Route cards lead with distance, not Valhalla's time estimate, since that estimate ignores live traffic unless TomTom is configured (see [Setup](docs/SETUP.md)).
- **Kochi Metro + Kochi Water Metro + Metro Connect feeder bus routing**, using bundled real station/schedule/fare data — no self-hosted transit server needed. Transit mode plans walk-or-drive-to-station, ride, walk-or-drive-to-destination itineraries from each system's own timetable, including combined Metro+Water-Metro trips through a real transfer point and Metro+feeder-bus trips to places the rail line itself doesn't reach (the airport, Infopark, CUSAT, ...). Supports stops (same "Add stop" UI as drive/walk), and shows up to 3 ranked alternatives as tappable cards, each with the real next-departure time and fare. Tap "Start" for live GPS tracking through every leg: a real turn-by-turn banner on walk/drive portions, a manual "I'm on the train"/"I'm on the boat" button (or automatic proximity+schedule detection) to confirm boarding, then a "next stop"/stops-remaining readout (a percent-of-distance readout on the water metro). See [docs/KOCHI_TRANSIT.md](docs/KOCHI_TRANSIT.md) for where the data comes from, how alternatives are ranked, and its caveats. A self-hosted [OpenTripPlanner 2](https://www.opentripplanner.org/) instance can cover a different city's transit (planning/rendering only, no live tracking) on top of this.
...
- **Turn-by-turn navigation** — live position arrow, traveled-route dulling, screen stays awake, automatic arrival detection, survives a mid-drive reload, reroutes onto whatever road you actually took. Auto-enters **Picture-in-Picture** on the [Android shell](docs/ANDROID.md) when minimized mid-drive. While driving, a Google-Maps-style round speed-limit sign sits next to your live speed readout, sourced from OSM `maxspeed` tags via Valhalla; a limit Valhalla only guessed from road class (no real tag on that stretch) is shown de-emphasized rather than with the same confidence as a real sign. On the `android-auto` branch (experimental, personal/developer use only, not yet on `main`) it also projects to **Android Auto**: a real rendered map with the route and destination/stop pins, live turn-by-turn, voice through the car speakers, and search (along the route or a whole new destination) straight from the car screen — see the [Android Auto section](docs/ANDROID.md) of the Android shell doc.
...
- **Live traffic** — when configured, every route option (even a single one with no alternates) is compared against current TomTom traffic before you even start, so "Fastest" reflects real conditions rather than Valhalla's traffic-blind estimate. If the fastest option is congested somewhere Valhalla's own alternates don't route around, the app also tries forcing a path past it and adds a genuinely new "Avoids traffic" option when that's actually faster. While driving: an occasional "Heavy traffic ahead" indicator and a traffic-adjusted ETA; if a real alternate is genuinely faster, the app reroutes to it automatically (announced out loud).
