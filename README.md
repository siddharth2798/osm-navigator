...
- **Search** — place search with typo-tolerant fallback, plus one-tap category chips (petrol, EV charging, pharmacy, ATM, hospital, food, parking, hotels); "X near me" resolves "me" to your live GPS position. Category searches that come back empty from OSM/Nominatim (EV charging especially, see "Known limitations") fall back to TomTom's Places Search, when configured.
...
- `TOMTOM_FEATURES_ENABLED` — optional. Gates two independent features: live traffic during drive navigation (TomTom Flow Segment Data, 20K free requests/month — see the `TRAFFIC_*` tunables in `config.js`) and a Places Search fallback for category searches that come back empty from Nominatim (5K–10K free requests/month depending on endpoint). Deliberately does **not** hold an API key — the real key never touches the client at all. Instead it lives only as a Cloudflare Pages secret (dashboard → Settings → Environment variables → `TOMTOM_API_KEY`), read server-side by two small Cloudflare Pages Functions (`functions/api/traffic.js`, `functions/api/places.js`) that proxy the actual TomTom calls. Get a free key at [developer.tomtom.com](https://developer.tomtom.com), set it as that Cloudflare secret, deploy, then flip this flag to `true`. Leave `false` to disable both features entirely — no calls to `/api/traffic` or `/api/places` at all.
...
One more thing that'll need attention before the native shell is truly usable day-to-day: `TOMTOM_FEATURES_ENABLED`'s `/api/traffic` and `/api/places` calls are relative, same-origin paths — correct for the web PWA (served from your actual Cloudflare Pages domain), but the packaged Android app isn't served from that domain, so those calls won't resolve there as-is. Fixing it means making the URL absolute (e.g. a `CONFIG.SITE_ORIGIN` pointing at your deployed domain) for native builds specifically — not done here since the native shell overall is still an unverified first draft (see above).
...
- **EV charging search is genuinely sparse** in India's current OSM coverage — a "no EV charging found nearby" result often reflects real data gaps, not a search bug. Enabling `TOMTOM_FEATURES_ENABLED` (see "Configuring your own services") adds a fallback to TomTom's Places Search for any category search Nominatim comes back empty on, which meaningfully helps here.
