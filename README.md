...
- **A button seems to do nothing, or something fails silently**: turn on **Debug mode** (Help & documentation → Developer tools, or `?debug=resolver` in the URL) and retry — every `console.log`/`warn`/`error` anywhere in the app (not just this app's own instrumentation — any library it loads too), plus any uncaught error, appears on screen instead of only being visible in devtools. This is the only practical way to see what actually happened on a real Android device, where devtools isn't reachable. The debug console's own **End** button turns Debug mode back off — its "✕" only dismisses the console for now, it'll reopen on the next log line.
- **The Debug log panel wouldn't close (or its close button closed something else instead) while a place was selected or navigation was active**: this was a real bug, now fixed — the panel used to be tracked on the same back-stack as every other overlay, so selecting a place afterward could push a new layer on top of it, and starting navigation could overwrite its entry outright, either way leaving its own close button routed to the wrong thing while the panel stayed stuck open. The panel (and its own close controls) now works independently of whatever else is open.
- **The "turn Debug mode off" button used to float top-left over the whole app, even on top of unrelated screens like Help & documentation**: this was a real bug, now fixed — it's a plain "End" button inside the debug console's own header instead, so it's only ever visible while the console itself is open.
- **The bottom sheet's collapsed "peek" state cropped the walk-mode elevation chart, cutting off its title/ascent-descent line, both while planning and during active navigation**: this was a real bug, now fixed — the elevation chart only exists once its own separate `/height` request resolves, which happens after the sheet's peek height was already measured for the route summary and buttons alone; nothing re-measured it once the chart actually appeared, so the sheet's `overflow: hidden` clipped whatever didn't fit the earlier, smaller measurement. Rendering (or hiding) the chart now re-measures the peek height immediately after, and that measurement itself now accounts for the chart's own height too.
- **The back button (hardware/gesture back on the Android shell, or the browser's back button on the web/PWA) used to skip straight past search suggestion dropdowns and an in-progress "set Home/Work" pick instead of closing just that one thing**: this was a real bug, now fixed — every suggestions dropdown (the plain search box, the from/to fields, and each stop's own field) and the invisible "pick a place to set as Home/Work" mode now register on the same back-stack as every other panel, so a single back press closes exactly one of them at a time, the same as it already did for the place card, directions mode, and everything else. One pre-existing, unrelated quirk remains on the web/PWA specifically (not on the Android shell): if a dropdown is dismissed by some other action (picking a result, tapping away) rather than by pressing back itself, one extra harmless back press may be silently absorbed with no visible effect before the next real step closes — a deliberate, long-standing tradeoff (see the comment on `forgetBackLayerIfTop` in `app.js`) rather than a bug, and it never accumulates beyond one.
- **Tapping "Add stop" and then a category chip (Petrol, EV charging, ...) to pick that stop left an empty, permanently-blank stop field behind, with the picked place added as a separate extra stop instead**: this was a real bug, now fixed — the category chips shown once a route is planned always appended the pick as a brand-new stop, with no awareness that "Add stop" had just opened an empty one specifically to be filled by exactly that pick. Picking a category result now fills the most recently added empty stop field if one is open, and only falls back to adding a new stop when there isn't one.
...
- **Elevation profile shows "Mostly flat, ↑0m ↓0m" on a walking route with real hills, even though the routing/turn-by-turn itself is correct**: this was a real, confirmed issue — a self-hosted Valhalla whose tiles were built without ever running `valhalla_build_elevation` against downloaded DEM data (a separate, easy-to-skip step from just building the routing graph) doesn't error on `/height`, it silently returns a flat value for every point regardless of the real terrain, and the app has no way to tell that apart from a route that's genuinely flat just by looking at the response. Verified live: the same coordinates against the public Valhalla server showed a real ~30m elevation swing that the self-hosted server reported as completely flat. The app now detects a self-hosted elevation answer where every point is identical and automatically retries that one chart against the public server (routing itself stays wherever it already was) — Debug mode shows a "self-hosted elevation came back completely flat... retrying" line when this kicks in. This masks the symptom in the app; the underlying self-hosted server still has no real elevation data until you rebuild it with `additional_data.elevation` configured.
...
- **Elevation profile** depends on Valhalla's `/height` action being enabled on whichever server you point at. If your self-hosted server's tiles were built without elevation data, the app detects the resulting flat profile and automatically retries that chart against the public server instead (see the troubleshooting entry below) — but that's a fallback, not a fix for the self-hosted server itself.
