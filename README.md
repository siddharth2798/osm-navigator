...
- **EV charging station details**, when `OPENCHARGEMAP_ENABLED` is turned on — connector type, power, operator, cost, and an honestly-labeled operational status ("Reported operational · checked 4 months ago", not a live "in use" signal — see the config comment for why). Falls back to a plain OSM pin with no detail otherwise.
...
- `OPENCHARGEMAP_ENABLED` — off by default; EV charging search stays a plain OSM pin with no connector/power/cost/status detail until turned on. Deliberately a two-step opt-in, not a single config value: set this to `true`, **and** set a free [Open Charge Map API key](https://openchargemap.org/site/develop/api) as a Cloudflare secret — `wrangler secret put OPENCHARGEMAP_API_KEY` for a plain Worker, or your Pages project's Settings → Environment variables (as a secret, not a plain variable) for Cloudflare Pages. The key never goes in `config.js`, since that file ships to every visitor's browser — the app calls this deployment's own `/api/opencharge-poi` instead, which attaches the key server-side (see `lib/opencharge-poi.js`). Only works on the Cloudflare Worker/Pages deployment paths (same requirement as the Google Maps link resolver below) — no server-side hop on plain static hosting. Not a live "is it free right now" feature either way — see the config comment for why (short version: that data sits behind OCPI, which requires a registered business relationship with each charging network, not something a personal project can get).
