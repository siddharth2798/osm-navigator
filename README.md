...
- **Live traffic during driving** — an occasional "Heavy traffic ahead" indicator and a traffic-adjusted ETA, backed by TomTom's Flow Segment Data, when configured. If a real alternate is genuinely faster, the app reroutes to it automatically (announced out loud) rather than just recalculating the same route.
