...
- **Directions** — multi-stop routing (up to 8, drag to reorder — including dragging a stop above the start or below the destination to promote it, swapping places with whichever it replaces), a plain-text "X to Y" shortcut, Drive/Walk/Transit modes, avoid-tolls/avoid-highways. Route cards lead with distance, not Valhalla's time estimate — that estimate has no live traffic behind it, unless TomTom live traffic is configured (see [Setup](docs/SETUP.md)).
- **Kochi Metro + Kochi Water Metro + Metro Connect feeder bus routing**, bundled with real station/schedule/fare data — no self-hosted transit server needed. Transit mode plans real walk-or-drive-to-station, ride, walk-or-drive-to-destination itineraries using each system's own actual timetable — including genuine combined Metro+Water-Metro trips through a real transfer point, and Metro+feeder-bus trips to places the rail line itself doesn't reach (the airport, Infopark, CUSAT, ...), whichever beats a single network alone — plus stops (same "Add stop" UI as drive/walk) — and surfaces up to 3 ranked alternatives as tappable cards for a stop-free trip, each showing the real next-departure time and fare on its ride leg. Tap "Start" on a planned trip for live GPS tracking through every leg — a real turn-by-turn banner on the walk/drive portions, a manual "I'm on the train"/"I'm on the boat" button (or automatic proximity+schedule detection) to confirm boarding, then a "next stop"/stops-remaining readout with a full scrollable station-by-station list on the metro, or a percent-of-distance readout on the water metro — the same live-puck experience drive/walk mode already has. See [docs/KOCHI_TRANSIT.md](docs/KOCHI_TRANSIT.md) for where the data (and fares) come from, how itinerary alternatives are ranked, how live tracking decides you've boarded, and its caveats. A self-hosted [OpenTripPlanner 2](https://www.opentripplanner.org/) instance can cover a different city's transit (planning/rendering only, no live tracking) on top of this.
