...
- **`functions/api/resolve-maps-url.js`** (Cloudflare Pages only) is what lets a pasted `maps.app.goo.gl` short link resolve — it just follows the redirect server-side, since a browser can't read a cross-origin redirect's target itself. No API key or secret to configure, Cloudflare Pages deploys it automatically with the rest of the site. GitHub Pages has no server functions at all, so a short link can't be resolved there; a full `google.com/maps` link still works either way, since those already carry the coordinates in the URL itself with nothing to resolve.
