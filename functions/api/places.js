...
// ALLOWED_TERMS plus TomTom's own key) and keeps this endpoint usable from
// a cross-origin caller (e.g. the Android shell) without a separate code path.
...
export async function onRequestGet(context) {
  ...
  const radius = Math.max(100, Math.min(parseInt(url.searchParams.get('radius'), 10) || 5000, 20000));
