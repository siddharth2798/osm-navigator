...
// Fixed bounding box for the `region=india` mode (Flight Tracking Mode /
// idle-mode browsing — see CONFIG.FLIGHT_REGION_POLL_INTERVAL_MS in
// config.js, this app's own client). Not reachable via point+radius —
// airplanes.live's radius cap (MAX_RADIUS_NM above) is a fraction of
// India's ~1600nm span, and even OpenSky's bboxFromPoint would need an
// enormous radius to cover a box this size accurately. OpenSky charges by
// bbox area (~900 sq° here, its top tier, 4 credits/query) — the reason
// this mode is polled far slower client-side than a normal point query.
...
// The self-hosted relay (self-hosted/flights-relay.mjs) gets its own,
// longer budget — confirmed live this session: with the same 5s timeout
// as the direct tiers, a relay response arriving anywhere near that
// window got aborted by the Worker before the relay even finished its
// own (up to 8s per attempt) OpenSky/airplanes.live tiering, then fell
// through to the direct tiers — which are known-blocked — for a
// guaranteed 502. 10s comfortably covers a real round trip through the
// tunnel plus the relay's own tiering on a slow moment, while the total
// worst case (relay timeout + both direct-tier timeouts) still stays
// under the ~20s window that produced the original 522.
 ...
 * Takes a bbox directly (not lat/lon/radius) so both the point-query and
 * region-query callers below share this one implementation — see
 * tryFetchOpenSky/nearbyFlightsRegion for the two ways in. */
async function fetchOpenSkyBbox({ lamin, lamax, lomin, lomax }, env) {
...
async function tryFetchOpenSky(lat, lon, radiusNm, env) {
  return fetchOpenSkyBbox(bboxFromPoint(lat, lon, radiusNm), env);
}

/** A self-hosted relay (see self-hosted/flights-relay.mjs), tried before
 * either direct upstream when env.SELF_HOSTED_FLIGHTS_URL is set. Exists
 * because, confirmed live this session: OpenSky's own GitHub README
 * states "We may block AWS and other hyperscalers due to generalized
 * abuse from these IPs" — Cloudflare Workers' shared egress falls under
 * the same policy. Both OpenSky and airplanes.live answer instantly from
 * a normal residential/VPS IP, but from inside this deployed Worker
 * OpenSky hangs (consistent with a network-level DROP) and airplanes.live
 * rejects fast (consistent with an app-layer block) — every single time,
 * regardless of tier order or auth. There is no Cloudflare-side fix for
 * this (Dedicated CDN Egress IPs is Enterprise-only, and is for inbound
 * traffic to your own origin anyway, not a Worker's own outbound fetch to
 * a third party). Same shape as tryFetchOpenSky — returns an already-
 * parsed `{ac: [...]}` body, since the relay itself already did the
 * OpenSky/airplanes.live tiering and shape conversion server-side, on a
 * box that isn't blocked. `queryString` is either `lat=..&lon=..&radiusNm=..`
 * (point mode) or `region=india` (region mode) — the relay itself
 * understands both, mirroring nearbyFlights' own two modes below. */
async function tryFetchSelfHostedRelay(queryString, env) {
  if (!env || !env.SELF_HOSTED_FLIGHTS_URL) return { ok: false, res: null };
  try {
    const res = await fetchWithTimeout(`${env.SELF_HOSTED_FLIGHTS_URL}/flights?${queryString}`, {
      headers: { 'x-relay-secret': env.RELAY_SHARED_SECRET || '' },
    }, RELAY_TIMEOUT_MS);
    if (!res.ok) return { ok: false, res };
    const body = await res.json();
    return { ok: true, body, sourceLabel: res.headers.get('x-flight-source') || 'self-hosted' };
  } catch (err) {
    return { ok: false, res: null };
  }
}
...
export async function nearbyFlights(requestUrl, env) {
  // Flight Tracking Mode / idle-mode browsing use this instead of
  // lat/lon/radiusNm — see INDIA_BBOX and nearbyFlightsRegion below.
  if (requestUrl.searchParams.get('region') === 'india') return nearbyFlightsRegion(env);
  ...
  // Self-hosted relay first, if configured (see tryFetchSelfHostedRelay's
  // own comment above) — the only path proven to actually reach either
  // upstream from this deployment. Falls through to the direct tiers
  // below when it's not configured, or when it itself fails, so a clone
  // of this repo with no relay set up still gets the same behavior as
  // before (degraded, but not silently different).
  const relay = await tryFetchSelfHostedRelay(`lat=${lat}&lon=${lon}&radiusNm=${radiusNm}`, env);
  let final = relay;
  let source = relay.sourceLabel;
  if (!relay.ok) {
    ...
    // deployment's Worker runtime), airplanes.live as the fallback. Both
    // are known-unreliable from this Worker's own egress when no relay is
    // configured — see tryFetchSelfHostedRelay's comment — but still worth
...
// Cached longer than a point query (CACHE_TTL_S) — this is polled far
// less often client-side (CONFIG.FLIGHT_REGION_POLL_INTERVAL_MS, ~100s)
// and each OpenSky call spends real credits, so a burst of near-
// simultaneous requests (multiple tabs/devices) shouldn't each pay for
// their own India-wide query.
const REGION_CACHE_TTL_S = 30;

/** India-wide traffic for Flight Tracking Mode / idle-mode browsing — see
 * INDIA_BBOX. No airplanes.live fallback: its point+radius API can't
 * cover a country-scale area at all (MAX_RADIUS_NM above is a fraction of
 * India's span), so there's nothing useful to fall back to — OpenSky's
 * bbox-native query is the only source that fits this shape. */
async function nearbyFlightsRegion(env) {
  const cache = typeof caches !== 'undefined' ? caches.default : null;
  const cacheKey = new Request('https://flights-proxy.internal/region/india');
  if (cache) {
    const cached = await cache.match(cacheKey);
    if (cached) return cached;
  }

  const relay = await tryFetchSelfHostedRelay('region=india', env);
  let final = relay;
  let source = relay.sourceLabel;
  if (!relay.ok) {
    final = await fetchOpenSkyBbox(INDIA_BBOX, env);
    source = 'opensky';
    if (!final.ok) {
      console.error('[flights-proxy] India-wide OpenSky query failed (status', final.res ? final.res.status : 'network error', ')');
    }
  }
  ...
  if (cache && final.ok) {
    const toCache = response.clone();
    toCache.headers.set('Cache-Control', `public, max-age=${REGION_CACHE_TTL_S}`);
    await cache.put(cacheKey, toCache);
  }
  return response;
}
