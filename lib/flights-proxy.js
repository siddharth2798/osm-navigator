...
// adsb.fi tier. UNLIKE adsb.lol/adsb.fi, it gates access behind manual
// approval: confirmed live (repeatedly, as of the date of this comment)
// HTTP 403 with "Please contact us at contact@airplanes.live" on every
// request, regardless of User-Agent — this tier does nothing useful on
// THIS deployment until that approval is granted (aurora's own deployment
// already has it — that's presumably why airplanes.live was its choice of
// primary source too). Kept as tier 1 anyway, matching aurora's own
// service/order exactly, since OpenSky below covers the gap until then —
// the moment approval lands, this starts actually answering with zero
...
// of intermittent rate-limiting/timeouts on this deployment. Anonymous
...
// tier) — a real, easy-to-hit cap under continuous polling, unlike
// airplanes.live once approved (see aurora's own experience). Coverage
...
export async function nearbyFlights(requestUrl, env) {
  ...
  if (!primary.ok) {
    ...
    const fallback = await tryFetchOpenSky(lat, lon, radiusNm);
