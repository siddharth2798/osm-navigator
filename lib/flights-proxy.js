...
// doesn't, nothing more — no state, no rate limiting of its own, no
// business logic beyond bounds-checking the inputs.
//
// adsb.lol is an unofficial, community-run aggregator with no uptime
// guarantee, and its own docs say an API key (obtained by running your own
// receiver) may become required at some unspecified future point — see
// docs/FLIGHT_TRACKING.md. Fine for a personal branch; not something to
// build main-line reliability expectations around.
...
export async function nearbyFlights(requestUrl, env) {
  ...
  try {
    ...
    // distinguishably from this endpoint's own 400 above.
