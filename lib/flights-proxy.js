 ...
 * box that isn't blocked. */
async function tryFetchSelfHostedRelay(lat, lon, radiusNm, env) {
  if (!env || !env.SELF_HOSTED_FLIGHTS_URL) return { ok: false, res: null };
  try {
    const res = await fetchWithTimeout(`${env.SELF_HOSTED_FLIGHTS_URL}/flights?lat=${lat}&lon=${lon}&radiusNm=${radiusNm}`, {
...
export async function nearbyFlights(requestUrl, env) {
  ...
  const relay = await tryFetchSelfHostedRelay(lat, lon, radiusNm, env);
  let final = relay;
