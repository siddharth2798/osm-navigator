...
/** Whichever metro station (if any) sits at the same real-world spot as
 * `feederStationName` — purely coordinate-based, same convention as
 * findKochiTransferPoints, using `maxM` as "close enough to be the same
 * physical stop." Every route in vendor/kochi-feeder-bus.json has exactly
 * one endpoint at a metro station's own premises (that's the whole point
 * of a *feeder* bus), so this needs no path-finding — just confirm which
 * end that is. Returns null if `feederStationName` isn't a known feeder
 * stop, or isn't within `maxM` of any metro station. */
