...
export async function resolveMapsUrl(requestUrl) {
  ...
  if (targetUrl.protocol !== 'https:' && targetUrl.protocol !== 'http:') return new Response('Bad request', { status: 400, headers: CORS_HEADERS });
  if (!ALLOWED_HOSTS.has(targetUrl.hostname)) return new Response('Bad request', { status: 400, headers: CORS_HEADERS });
