...
export async function resolveMapsUrl(requestUrl) {
  ...
  try {
    ...
    // The allow-list is checked on the way IN (targetUrl above) and on the
    // interstitial-unwrap's own `continue` param, but `res.url` itself is
    // wherever the redirect chain actually ended up — normally another
    // ALLOWED_HOSTS domain (that's the whole point of following it), but a
    // compromised or since-repurposed short link could just as easily
    // redirect somewhere else entirely. Re-checking here closes that off:
    // this endpoint hands back a resolved Google Maps URL or nothing, never
